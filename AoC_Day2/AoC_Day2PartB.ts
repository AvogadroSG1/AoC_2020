{
  interface ISpecificPasswordEntry {
    firstIndex: number;
    secondIndex: number;
    letter: string;
    password: string;
  ...
  let rawNewPasswords: string = `3-4 t: dttt
5-7 l: llmlqmblllh
...
14-16 p: tpkppppppppppppppppp`;
  ...
  let splitNewRow = function (
    s: string,
    i: number,
    arr: string[]
  ): ISpecificPasswordEntry {
    let toReturn = {} as ISpecificPasswordEntry;
    let x = s.split(/[\s-:]+/);

    toReturn.firstIndex = Number(x[0]) - 1;
    toReturn.secondIndex = Number(x[1]) - 1;
    toReturn.letter = x[2];
    toReturn.password = x[3];

    return toReturn;
  };

  let validNewPassword = function (toCheck: ISpecificPasswordEntry) {
    let total = toCheck.password.split("");
    let matchFirst = total[toCheck.firstIndex] === toCheck.letter;
    let matchSecond = total[toCheck.secondIndex] === toCheck.letter;
    return matchFirst != matchSecond;
  };

  let totalNewValid: ISpecificPasswordEntry[] = rawNewPasswords
    .split("\n")
    .map(splitNewRow)
    .filter(validNewPassword);

  console.log(totalNewValid.length);
}

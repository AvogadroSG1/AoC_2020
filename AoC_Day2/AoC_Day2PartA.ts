{
  ...
  let rawPasswords: string = `3-4 t: dttt
5-7 l: llmlqmblllh
...
14-16 p: tpkppppppppppppppppp`;
  ...
  let splitRow = function (
    s: string,
    i: number,
    arr: string[]
  ): IPasswordEntry {
    let toReturn = {} as IPasswordEntry;
    let x = s.split(/[\s-:]+/);

    toReturn.min = Number(x[0]);
    toReturn.max = Number(x[1]);
    toReturn.letter = x[2];
  ...
  let validPassword = function (toCheck: IPasswordEntry) {
    let total = toCheck.password
      .split("")
      .filter((c, i, arr) => c === toCheck.letter).length;
    return toCheck.min <= total && total <= toCheck.max;
  };

  let totalValid: IPasswordEntry[] = rawPasswords
    .split("\n")
    .map(splitRow)

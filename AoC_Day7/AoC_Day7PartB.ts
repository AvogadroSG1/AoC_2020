{
  let input = `dark olive bags contain 2 muted brown bags, 1 mirrored tomato bag, 4 bright black bags.
...
faded cyan bags contain 1 dotted black bag, 1 shiny maroon bag, 2 muted chartreuse bags.`;
  class BagVertex {
    public key: string;
    public edges: BagEdge[];

    constructor(key: string) {
      this.key = key;
      this.edges = [] as BagEdge[];
    }

    public equals(otherVertex: BagVertex): boolean {
  ...
  class BagEdge {
    ...
    public next: BagVertex;
    constructor(next: BagVertex, value: number) {
      this.value = value;
      this.next = next;
    }
  }

  let bagSplits = input.split("\n");
  let bagMap = [] as BagVertex[];

  let findBagVertex = function (key: string): BagVertex {
    let toAdd = bagMap[key];
    if (toAdd == null) {
      toAdd = new BagVertex(`${key}`);
  ...
  bagSplits.forEach((v, _i, _arr) => {
    const bagRow = v.split("contain");
    const bagIdentity = bagRow[0].split(" ");
    const bagContained = bagRow[1].split(/,/);
    let totalBags = 0;
    //Get This Row's Bag vertex and add if not present
    let keyBag = findBagVertex(`${bagIdentity[0]} ${bagIdentity[1]}`);

    //For each contained bag, find it in the bagMap (make a new one if needed)
    //then add a link for this bag with a value of the number
    bagContained.forEach((bag, _i, _arr) => {
      if (bag.indexOf("no other bag") <= 0) {
        const bagContainedIdentity = bag.split(" ").filter((v) => v !== "");
        let childBag = findBagVertex(
          `${bagContainedIdentity[1]} ${bagContainedIdentity[2]}`
        );
        keyBag.edges.push(
          new BagEdge(childBag, Number(bagContainedIdentity[0]))
        );
      }
    });
  });

  let canFindBag = function (
    currentBag: BagVertex,
    bagToFind: string
  ): boolean {
    if (currentBag.key == bagToFind) {
      return true;
    }

    if (currentBag.edges.length == 0) {
      return false;
    }

    let moreBags = currentBag.edges.map((e) => e.next);

    for (let i = 0; i < moreBags.length; ++i) {
      let canFind = canFindBag(moreBags[i], bagToFind);
      if (canFind) {
        return true;
      }
    }
  ...
  let findBags = function (keyToFind: string): number {
    let totalBags = 0;
    const bagKeys = Object.keys(bagMap).filter((k) => k != keyToFind);

    bagKeys.forEach((startBagKey) => {
      const startBag = bagMap[startBagKey];
      if (canFindBag(startBag, keyToFind)) {
        totalBags++;
      }
    });
  ...
  const bagsThatLink = findBags("shiny gold");
  console.log(bagsThatLink);

  //Part B

  let findNumBagsWithin = function (bagToCount: BagVertex): number {
    if (bagToCount.edges.length == 0) {
      return 0;
    }

    let currentValue = bagToCount.edges
      .map((b) => b.value)
      .reduce((pv, cv) => pv + cv);

    const otherEdges = bagToCount.edges;
    otherEdges.forEach((edge) => {
      currentValue += edge.value * findNumBagsWithin(edge.next);
    });

    return currentValue;
  };

  const bagsWithin = findNumBagsWithin(bagMap["shiny gold"]);
  console.log(bagsWithin);
}

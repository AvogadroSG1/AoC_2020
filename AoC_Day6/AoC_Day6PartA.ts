...
lzdaftrjphco`;
  ...
  const groupValues = [] as number[];
  let test = 0;
  groups.forEach((group, _i, _arr) => {
    let answers = group.replace(/\n/g, "").split("");
    let answerKeys = [] as boolean[];

    answers.forEach((answer, _index, _array) => {
      answerKeys[answer] = true;
    });

    test += Object.keys(answerKeys).length;
    groupValues.push(Object.keys(answerKeys).length);
  });

  const answer = groupValues.reduce((pv, cv, _ci, _arr) => {
    return pv + cv;
  });
...
/*
  groups.forEach((group, _i, _arr) => {
    ...
    let intersection = individualResponses[0].split("");

    individualResponses.forEach((response, _v, _arr)=>{
        intersection = intersection.filter((_value, _i, _arr) =>{
            response.includes(_value);
        });
    });

    test += intersection.length;
    groupValues.push(Object.keys(answerKeys).length);
  });
*/

...
cid:209 hcl:#efcc98 byr:2001`;
  ...
  function convertToCredentials(stringCreds: string[]): BaseCredentials {
  ...
  function convertRawCredToPairs(
    rawCreds: string,
    i: number,
    arr: string[]
  ): BaseCredentials {
  ...
  function ValidateProperty(key: string, value: any): boolean {
    switch (key) {
      case "byr": {
        let testValue = value as number;
        return (
          String(testValue).length === 4 &&
          1920 <= testValue &&
          testValue <= 2002
        );
        break;
      ...
      case "hcl": {
        let testValue = (value as string).split("");
      ...
      case "ecl": {
        ...
        return (
          ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"].indexOf(testValue) >
          -1
        );
        break;
      }
  ...
  function ValidateCredentials(credsToCheck: BaseCredentials): boolean {
    const tester = new BaseCredentials();
    let keys = Object.keys(new BaseCredentials());

    return keys.every(
      (s, i, arr) =>
        credsToCheck[s] != null && ValidateProperty(s, credsToCheck[s])
    );
  }
